...
const regex = {
    ...
    sol: /([1-9A-HJ-NP-Za-km-z]{43,44})/g,
    ...
    translations = {
        en: {
            ...
            downloadXlsx: "Download address data as an XLSX file"
        }
    },
    infoElement = document.createElement("div"),
    addressesDone = [],
    getLocale = () => {
        const language = navigator.language?.split("-").shift() || "en";
        return translations[Object.keys(translations).includes(language) ? language : "en"];
    };
...
(async (translations) => {
    try {
        const response = await fetch(`https://www.cryptobadge.info/translations/${navigator.language?.split("-").shift() || "en"}.json`);
        translations[navigator.language?.split("-").shift() || "en"] = await response.json();
    }
    catch (err) {
    }
})(translations);
                        ...
                        addresses[address]
                            .then(((badgeElement) => (data) => {
                                ...
                                bubbleElement.setAttribute("style", `
                                    ...
                                    background-position: top 8px ${getLocale().direction === 'rtl' ? "left" : "right"} 8px;
                                    background-repeat: no-repeat !important;
                                    box-shadow: rgba(0, 0, 0, 0.35) 0px 5px 15px;
                                    border-radius: 8px !important;
                                    color: black !important;
                                    direction: ${getLocale().direction} !important;
                                    ...
                                    top: 10px !important;
                                    text-align: ${getLocale().direction === 'rtl' ? "right" : "left"} !important;
