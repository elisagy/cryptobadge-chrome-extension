...
const regex = {
    ...
    locale = ((locales) => {
        ...
        return locales[Object.keys(locales).includes(language) ? language : "en"];
    })({
        ar: {
            direction: "rtl",
            address: "العنوان",
            asset: "الأصل",
            balance: "الرصيد",
            firstTransactionDate: "تاريخ أول معاملة",
        ...
        es: {
            ...
            asset: "activo",
            balance: "saldo",
            firstTransactionDate: "fecha de la primera transacción",
            lastTransactionDate: "fecha de la última transacción",
        ...
        he: {
            ...
            asset: "נכס",
            balance: "יתרה",
            firstTransactionDate: "תאריך העסקה הראשונה",
            lastTransactionDate: "תאריך העסקה האחרונה",
        ...
        ru: {
            ...
            asset: "актив",
            balance: "баланс",
            firstTransactionDate: "дата первой транзакции",
                        ...
                        addresses[address]
                            .then(((badgeElement) => (data) => {
                                ...
                                bubbleElement.setAttribute("style", `
                                    ...
                                    background-position: top 8px ${locale.direction === 'rtl' ? "left" : "right"} 8px;
                                    background-repeat: no-repeat !important;
                                    box-shadow: rgba(0, 0, 0, 0.35) 0px 5px 15px;
                                    border-radius: 8px !important;
                                    color: black !important;
                                    direction: ${locale.direction} !important;
                                    ...
                                    top: 10px !important;
                                    text-align: ${locale.direction === 'rtl' ? "right" : "left"} !important;
                                ...
                                if (data) {
                                    ...
                                    bubbleElement.innerHTML = `<b style="pointer-events: none !important;">${locale.address}:</b> ${data.address}<br><b style="pointer-events: none !important;">${locale.asset}:</b> ${data.asset}<br><b style="pointer-events: none !important;">${locale.balance}:</b> ${data.balance}${price ? ` (~${formatter.format(price.last_trade_price * data.balance)})` : ``}<br><b style="pointer-events: none !important;">${locale.firstTransactionDate}:</b> ${(new Date(data.firstTransactionDate)).toString()}<br><b style="pointer-events: none !important;">${locale.lastTransactionDate}:</b> ${(new Date(data.lastTransactionDate)).toString()}<br><b style="pointer-events: none !important;">${locale.lastUpdate}:</b> ${(new Date(data.updatedDate)).toString()}${data.graphsDataURIs?.map(graphDataURI => `<br><br><img src="${graphDataURI}" style="pointer-events: none !important;">`).join("")}`;
                                }
                                else {
